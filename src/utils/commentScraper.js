...
/** Key used to match an API comment to a DOM node */
function commentKey(comment) {
  return `${normalise(comment.author)}::${normalise(comment.text).slice(0, 80)}`;
}

/**
 * Returns a Set of keys for every comment currently rendered in the YouTube DOM.
 * Used to determine whether an API comment can be scrolled to right now.
 */
export function buildRenderedSet() {
  const keys = new Set();
  ...
  for (const node of nodes) {
    ...
    const key = `${normalise(authorEl.innerText)}::${normalise(textEl.innerText).slice(0, 80)}`;
    keys.add(key);
  }
  return keys;
}

/** Returns true if the given API comment is currently rendered in the YouTube DOM */
export function isCommentRendered(comment, renderedSet) {
  return renderedSet.has(commentKey(comment));
}
